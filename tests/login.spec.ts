...
test.describe('Login Screen Functionality', { tag: ['@E2E'] }, () => {
    ...
    test.describe('UI and validation functionality', { tag: ['@login'] }, () => {

        test('TC_LOGIN_01 : Verify the systems response when attempting to log in without entering credentials.', { tag: ['@smoke'] }, async ({ page }) => {
            await page.getByRole('button', { name: 'Login' }).click();
            await expect(page.getByText('Please enter a valid email')).toBeVisible();
            await expect(page.getByText('Password must be at least 8')).toBeVisible();
        });

        test('TC_LOGIN_02 : Verify the systems response when attempting to log in with only the email details entered.', { tag: ['@smoke'] }, async ({ page }) => {
            await page.getByRole('textbox', { name: 'Enter your email' }).fill('agent@gmail.com');
            await page.getByRole('button', { name: 'Login' }).click();
            await expect(page.getByText('Password must be at least 8')).toBeVisible();
        });

        test('TC_LOGIN_03 : Verify the systems response when attempting to log in with only the password details entered.',{ tag: ['@smoke'] }, async ({ page }) => {
            await page.getByRole('textbox', { name: 'Enter your password' }).fill('Smartwork@123');
            await page.getByRole('button', { name: 'Login' }).click();
            await expect(page.getByText('Please enter a valid email')).toBeVisible();
        });

        test('TC_LOGIN_04 : Verify the systems response when attempting to log in with invalid credentials.', { tag: ['@smoke'] }, async ({ page }) => {
        ...
        test('TC_LOGIN_05 : Verify successful login to the Trip Vista application with valid credentials.',{ tag: ['@smoke'] },  async ({ page }) => {
        ...
        test('TC_LOGIN_06 : Verify the Sign-Up link in the login page.', { tag: ['@smoke'] }, async ({ page }) => {
            await expect(page.getByRole('button', { name: 'Sign up' })).toBeVisible();
            await page.getByRole('button', { name: 'Sign up' }).click();
            await expect(page.getByText('Agent Register')).toBeVisible();
        });

        test('TC_LOGIN_07 : Verify that the "Eye" icon in the password field toggles the visibility of the entered password correctly.', { tag: ['@smoke'] }, async ({ page }) => {
            await page.getByRole('textbox', { name: 'Enter your email' }).fill('agent@gmail.com');
        ...
        test('TC_LOGIN_08 : Verify usernames & MyBalance are displayed correctly in the right-side corner of the Dashboard Page.', { tag: ['@smoke'] }, async ({ page }) => {
            await loginCredentials(page, 'agent@gmail.com', 'SmartWork@1234');
